...
def load_dataset(dataset_name='MNIST', val_percentage = 0.3, batch_size = 64):
    ...
    cifar10_classes = set(ds_train.classes)
    print(f"CIFAR-10 classes: {cifar10_classes}")

    cifar100_classes = set(cifar100_train.classes)
    ood_classes = list(cifar100_classes - cifar10_classes)
    print(f"Out-of-distribution classes: {ood_classes}")

    # Subset CIFAR-100 to include only OOD classes
    ood_train_indices = [i for i, target in enumerate(cifar100_train.targets) if target in ood_classes]
    ood_train_subset = Subset(cifar100_train, ood_train_indices)
    ood_test_indices = [i for i, target in enumerate(cifar100_test.targets) if target in ood_classes]
    ood_test_subset = Subset(cifar100_test, ood_test_indices)
    ...
    return train_loader, val_loader, test_loader, ood_test_subset
